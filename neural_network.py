...
def train_test_splitter(target, features, id_column, train_size):
    ...
    for c in classes:
        ...
        temp_data = full[full['y'] == c].iloc[:int(mt.floor(train_size * class_full)), :]
...
def one_hot_encoder(dependent_var):
    encoded_data = pd.get_dummies(dependent_var)
    return encoded_data
...
y_train, x_train, y_test, x_test = train_test_splitter(target=raw_target,
